  ...
  mongoose.connection
    .once("open", () => {
      try {
        ...
        return new Error("There was a problem with one of your callbacks");
      }
    })
    .on("error", (error) => {
...
export const useDB = (cb: Function) => {
  ...
  mongoose.connection
    .once("open", () => {
      console.log(
        `database connection opened to MongoDB on port ${mongoose.connection.port}`
      );
      try {
        cb();
      } catch (error) {
    ...
    .on("close", () => {
      console.log(
        `database conneciton closed on port: ${mongoose.connection.port}`
      );
    });
};
